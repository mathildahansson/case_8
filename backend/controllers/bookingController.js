...
import Show from '../models/Show.js'; // importera show-modellen då shows uppdateras direkt
...
export const createBooking = async (req, res) => {
  try {
    ...
    // kontrollera att showen finns
    const existingShow = await Show.findById(show);
    if (!existingShow) {
      return res.status(404).json({ error: 'den valda föreställningen kunde inte hittas...' });
    }

    // kontrollera om platserna är lediga
    const invalidSeats = seats.filter(seat => !existingShow.availableSeats.includes(seat));
    if (invalidSeats.length > 0) {
      return res.status(400).json({ error: `platserna ${invalidSeats.join(', ')} är inte tillgängliga...` });
    }
